...
#endif
{
  ...
  if (!ServoInit) {
    ServoPin = Sevo_Pin;
    mRgbUltrasonic = new RgbUltrasonic(ServoPin);
    ServoInit = true;
  }
}

#if ARDUINO > 10609
void Hummerbot::SetFanPin(uint8_t Fan_Pin = EM_FAN_PIN,uint8_t State=LOW)
#else
void Hummerbot::SetFanPin(uint8_t Fan_Pin,uint8_t State)
#endif
{
